...
object Event {
  ...
  final case class UserEvent(
    ...
    def eventId: String = s"$campaignId-$userId-$timestamp"
  }
  ...
  implicit val eventDecoder: Decoder[Event] = List(Decoder[UserEvent].widen[Event], Decoder[CampaignEvent].widen[Event]).reduceLeft(_ or _)
